	...
	public class SpringLoadedTree extends Tree
	{
	    ...
	    * */    
		public function SpringLoadedTree(){
			...
			//change labelField, refreshData
			ChangeWatcher.watch(this, "dataProvider", 
				refreshTree);
		}
		/**
		 * refresh tree
		...
		private function refreshTree(event:Event):void{
					if(this.labelField !== null && this.dataProvider !== null) 
						this.dataProvider = this.dataProvider as ArrayCollection;
				}
		...
		* Handle the delayed call to close any un wanted nodes.  
	 	* This is called in a few areas to properly handle the closing.
	 	 * 
	 	 * @param item:default value is null
		 * @return 
		**/
		...
		**/
		private function handleDragOver(event:DragEvent):void{
			...
			if (currNodeOver !==null){
				...
				if (!isItemOpen(currNodeOver.data)){
				
					//If it's already running on the current item avoid a timer reset.
					if (_delayedTimer.running && _delayedTimer.item ===currNodeOver.data){
						return;
					}else if (_delayedTimer.running) {
		...
		private function restoreState(event:TimerEvent):void{
			...
			for (var i:int = 0; i < currentOpenFolders.length; i++){
				if (openedFolderHierarchy !== null && openedFolderHierarchy.indexOf(currentOpenFolders[i])===-1){
					if (dataDescriptor.isBranch(currentOpenFolders[i]) && !dataDescriptor.hasChildren(currentOpenFolders[i])){				
						try{
					...
					}else if (dataDescriptor.isBranch(currentOpenFolders[i]) && dataDescriptor.hasChildren(currentOpenFolders[i])){
						try{
		...
		**/
		...
		/**
		 * modify this to accept anything from any list to be able to be dropped
		 * manual adjusting will have to be done on the component initiating this class 
		 * to get the items to actually add the items to the provider though
		 * 
		 * @param event
		 * @return
		 */
		override protected function dragOverHandler(event:DragEvent):void{
			...
			if (event.dragSource.hasFormat("treeItems") || event.dragInitiator is ListBase){
